...
3. 进行手动筛选，筛选出需要的变量
4. 将筛选后的数据导出为csv
5. 按照手动筛选出的变量名对原始数据进行清洗，去除不需要的变量
...
# 导出成parquet格式
write_parquet(my_data_raw, "my_data_raw.parquet")
...
write_parquet(my_data_filtered_100_ic, "filtered_data_100_ic.parquet")
...
write_parquet(my_data_filtered_80_percent_ic, "filtered_data_80_percent_ic.parquet")
```
## 自变量筛选
### 自变量及其标签导出
```{r 自变量及其标签导出}
...
# 保存Excel文件
saveWorkbook(wb, "variable_labels.xlsx", overwrite = TRUE)

```
### 临床经验筛选
```{r 临床经验筛选}
# 读取变量列表
variable_list <- read.csv("variable_labels_filtered.csv")
...
# 导出自变量手动筛选后的数据
write_parquet(my_data_manual_filtered, "my_data_manual_filtered.parquet")
...
# 查看训练集和测试集的大小
cat("训练集大小:", nrow(trainData), "\n")
cat("测试集大小:", nrow(testData), "\n")
```
