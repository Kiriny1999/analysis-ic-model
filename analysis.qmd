...
my_data_calculated <- my_data_calculated %>%
    mutate(
        height = round(select(my_data_manual_filtered, qi002), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        weight = round(select(my_data_manual_filtered, ql002), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        waist_circumference = round(select(my_data_manual_filtered, qm002), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        white_blood_cell = round(select(my_data_manual_filtered, bl_wbc), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        hematocrit = round(select(my_data_manual_filtered, bl_hct), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        mean_corpuscular_volume = round(select(my_data_manual_filtered, bl_mcv), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        platelets = round(select(my_data_manual_filtered, bl_plt), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        triglycerides = round(select(my_data_manual_filtered, bl_tg), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        creatinine = round(select(my_data_manual_filtered, bl_crea), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        blood_urea_nitrogen = round(select(my_data_manual_filtered, bl_bun), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        hdl_cholesterol = round(select(my_data_manual_filtered, bl_hdl), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        ldl_cholesterol = round(select(my_data_manual_filtered, bl_ldl), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        total_cholesterol = round(select(my_data_manual_filtered, bl_cho), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        glucose = round(select(my_data_manual_filtered, bl_glu), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        uric_acid = round(select(my_data_manual_filtered, bl_ua), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        cystatin_c = round(select(my_data_manual_filtered, bl_cysc), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        c_reactive_protein = round(select(my_data_manual_filtered, bl_crp), 2)
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        glycated_hemoglobin = round(select(my_data_manual_filtered, bl_hbalc), 2)
    )
