...
table(my_data_calculated$visithospital_lastmonth, useNA = "ifany")
...
table(my_data_calculated$reason_not_seeking_medical_attention, useNA = "ifany")
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        payfor_outpatient = case_when(
            times_visited_outpatient == 0 ~ 0,  # 如果门诊次数为0，则不付费case
            TRUE ~ case_when(
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        travel_time_outpatient = case_when(
            times_visited_outpatient == 0 ~ 0,  # 如果去门诊次数为0，时间为0
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        travel_cost_outpatient = case_when(
            times_visited_outpatient == 0 ~ 0,  # 如果没去过门诊，费用为0
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        emergency_outpatient = case_when(
            times_visited_outpatient == 0 ~ 0,  # 如果去门诊次数为0，则为0
            TRUE ~ case_when(
...
table(my_data_calculated$reasons_inpatientcare, useNA = "ifany")
