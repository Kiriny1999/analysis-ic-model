...
# 以下代码会读取指定文件夹下的所有 Stata 文件，为每个文件提取变量名与标签，
# 然后将其写入同一个 Excel 工作簿的不同工作表中
if (!requireNamespace("haven", quietly = TRUE)) install.packages("haven")
if (!requireNamespace("openxlsx", quietly = TRUE)) install.packages("openxlsx")
...
files_stata <- list.files("data_raw/2015/self", pattern = "\\.dta$", full.names = TRUE)
wb <- createWorkbook()

for (fn in files_stata) {
    data_name <- sub("\\.dta$", "", basename(fn))
    tmp_data <- read_dta(fn)
    v_names <- names(tmp_data)
    v_labels <- sapply(tmp_data, function(x) {
        lbl <- attr(x, "label")
    ...
    writeData(wb, data_name, df_out)
}

saveWorkbook(wb, "data_export/all_data_labels.xlsx", overwrite = TRUE)
```
### 临床经验筛选
```{r 临床经验筛选}
# 读取变量列表
    ...
    mutate(
        has_parents_alive = case_when(
            ...
            rowSums(select(my_data_manual_filtered, 
                ca000_w3_2_1_, ca000_w3_2_2_,
                ca001_w3_2_1_, ca001_w3_2_2_) == 1, na.rm = TRUE) > 0 ~ 1,
            
            # 检查本人的继父继母
            my_data_manual_filtered$ca002_w3_1_1_ == 1 & 
            my_data_manual_filtered$ca004_w3_1_1_ == 1 & 
            my_data_manual_filtered$ca004_w3_2_1_ == 1 ~ 1,
            
            my_data_manual_filtered$ca002_w3_1_1_ == 1 & 
            my_data_manual_filtered$ca005_w3_1_1_ == 1 & 
            my_data_manual_filtered$ca005_w3_2_1_ == 1 ~ 1,
            
            # 检查配偶的继父继母
            my_data_manual_filtered$ca002_w3_1_2_ == 1 & 
            my_data_manual_filtered$ca004_w3_1_2_ == 1 & 
            my_data_manual_filtered$ca004_w3_2_2_ == 1 ~ 1,
            
            my_data_manual_filtered$ca002_w3_1_2_ == 1 & 
            my_data_manual_filtered$ca005_w3_1_2_ == 1 & 
            my_data_manual_filtered$ca005_w3_2_2_ == 1 ~ 1,
            
            # 检查本人的养父母
            my_data_manual_filtered$ca006_w3_1_1_ == 1 & 
            my_data_manual_filtered$ca006_w3_2_1_ == 1 ~ 1,
            
            my_data_manual_filtered$ca007_w3_1_1_ == 1 & 
            my_data_manual_filtered$ca007_w3_2_1_ == 1 ~ 1,
            
            # 检查配偶的养父母
            my_data_manual_filtered$ca006_w3_1_2_ == 1 & 
            my_data_manual_filtered$ca006_w3_2_2_ == 1 ~ 1,
            
            my_data_manual_filtered$ca007_w3_1_2_ == 1 & 
            my_data_manual_filtered$ca007_w3_2_2_ == 1 ~ 1,
            
            # 如果以上条件都不满足且所有相关变量都是NA，则返回NA
            all(is.na(select(my_data_manual_filtered,
                ca000_w3_2_1_, ca000_w3_2_2_,
                ca001_w3_2_1_, ca001_w3_2_2_,
                ca002_w3_1_1_, ca002_w3_1_2_,
                ca004_w3_1_1_, ca004_w3_1_2_,
                ca004_w3_2_1_, ca004_w3_2_2_,
                ca005_w3_1_1_, ca005_w3_1_2_,
                ca005_w3_2_1_, ca005_w3_2_2_,
                ca006_w3_1_1_, ca006_w3_1_2_,
                ca006_w3_2_1_, ca006_w3_2_2_,
                ca007_w3_1_1_, ca007_w3_1_2_,
                ca007_w3_2_1_, ca007_w3_2_2_
            ))) ~ NA_real_,
...
attr(my_data_calculated$has_parents_alive, "label") <- "父母是否健在"
# 查看父母是否健在的数据分布
table(my_data_calculated$has_parents_alive, useNA = "always")

## 计算健在父母数量（貌似原始变量的取值定义错了，不是yes和no，而是 1和2，我在代码层面改了）
my_data_calculated <- my_data_calculated %>%
    mutate(
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        takecare_grandchildren_week = case_when(
            # 检查是否所有值都是NA
            all(is.na(select(my_data_manual_filtered, starts_with("cf003_1_")))) ~ NA_real_,
            # 否则计算非NA值的总和
            TRUE ~ rowSums(
                select(my_data_manual_filtered, starts_with("cf003_1_")),
                na.rm = TRUE  # 忽略部分NA值
            )
        )
    )
attr(my_data_calculated$takecare_grandchildren_week, "label") <- "每周照顾孙子女时间"

## 计算照顾孙子女小时数，这个是每周小时乘周数吗？
my_data_calculated <- my_data_calculated %>%
    mutate(
        takecare_grandchildren_hours = case_when(
            # 检查是否所有相关变量都是NA
            all(is.na(select(my_data_manual_filtered, 
                starts_with("cf003_2_"), starts_with("cf003_1_")))) ~ NA_real_,
            # 否则计算每个孙子女的照顾时间并求和
            TRUE ~ rowSums(
                cbind(
                ...
                na.rm = FALSE  # 如果有NA值，整行结果为NA
            )
        )
    )
attr(my_data_calculated$takecare_grandchildren_hours, "label") <- "每周照顾孙子女小时数"
...
attr(my_data_calculated$takecare_parents_hours, "label") <- "每周照顾父母小时数"

# 导出计算后的数据框
write_parquet(my_data_calculated, "data_export/my_data_calculated.parquet")
