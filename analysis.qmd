...
```{mermaid}
graph TD
    A["读取并合并数据"] --> B["计算步行测试时间"]
    B --> B1["判断步行测试时间是否为993"]
    B1 -->|是| B2["步行测试时间得分为0"]
    B1 -->|否| B3["判断步行测试时间是否为999或NA"]
    B3 -->|是| B4["判断步行测试未完成原因"]
    B4 -->|原因是4或6| B5["步行测试时间得分为0"]
    B4 -->|其他原因| B6["步行测试时间得分为NA"]
    B3 -->|否| B7["判断步行测试使用辅具情况"]
    B7 -->|使用辅具| B8["步行速度 >= 1米/秒得1分，否则得0分"]
    B7 -->|未使用辅具| B9["步行测试时间得分为NA"]
    B8 --> C["计算双脚半前后站立时间分数"]
    C --> C1["判断双脚半前后站立时间是否为993"]
    C1 -->|是| C2["双脚半前后站立时间得分为0"]
    C1 -->|否| C3["判断双脚半前后站立时间是否为999或NA"]
    C3 -->|是| C4["判断未完成原因"]
    C4 -->|原因是4或6| C5["双脚半前后站立时间得分为0"]
    C4 -->|其他原因| C6["双脚半前后站立时间得分为NA"]
    C3 -->|否| C7["判断是否完成双脚半前后站立测试"]
    C7 -->|完成| C8["双脚半前后站立时间得分为1"]
    C7 -->|未完成| C9["双脚半前后站立时间得分为0"]
    C8 --> D["计算双脚前后一条线站立时间分数"]
    D --> D1["判断双脚前后一条线站立时间是否为993"]
    D1 -->|是| D2["双脚前后一条线站立时间得分为0"]
    D1 -->|否| D3["判断双脚前后一条线站立时间是否为999或NA"]
    D3 -->|是| D4["判断未完成原因"]
    D4 -->|原因是4或6| D5["双脚前后一条线站立时间得分为0"]
    D4 -->|其他原因| D6["双脚前后一条线站立时间得分为NA"]
    D3 -->|否| D7["判断是否完成双脚前后一条线站立测试"]
    D7 -->|完成| D8["双脚前后一条线站立时间 >= 10秒得1分，否则得0分"]
    D7 -->|未完成| D9["双脚前后一条线站立时间得分为0"]
    D8 --> E["计算双脚并拢站立时间分数"]
    E --> E1["判断双脚并拢站立时间是否为993"]
    E1 -->|是| E2["双脚并拢站立时间得分为0"]
    E1 -->|否| E3["判断双脚并拢站立时间是否为999或NA"]
    E3 -->|是| E4["判断未完成原因"]
    E4 -->|原因是4或6| E5["双脚并拢站立时间得分为0"]
    E4 -->|其他原因| E6["双脚并拢站立时间得分为NA"]
    E3 -->|否| E7["判断是否完成双脚并拢站立测试"]
    E7 -->|完成| E8["双脚并拢站立时间得分为1"]
    E7 -->|未完成| E9["双脚并拢站立时间得分为0"]
    E8 --> F["计算最终平衡得分"]
    F --> F1["判断平衡测试是否受影响"]
    F1 -->|受影响| F2["最终平衡得分为0"]
    F1 -->|未受影响| F3["根据各项平衡测试得分计算最终平衡得分"]
    F2 --> G["计算坐起测试时间"]
    F3 --> G
    G --> G1["判断坐起测试时间是否为993"]
    G1 -->|是| G2["坐起测试时间得分为0"]
    G1 -->|否| G3["判断坐起测试时间是否为999或NA"]
    G3 -->|是| G4["判断未完成原因"]
    G4 -->|原因是4或6| G5["坐起测试时间得分为0"]
    G4 -->|其他原因| G6["坐起测试时间得分为NA"]
    G3 -->|否| G7["判断是否完成坐起测试"]
    G7 -->|完成| G8["坐起测试时间 <= 12秒且未使用手臂得1分，否则得0分"]
    G7 -->|未完成| G9["坐起测试时间得分为0"]
    G8 --> H["计算运动维度总分"]
    H --> I["导出运动维度所有变量和ID"]
```
...
```{mermaid}
graph TD
    A["读取并合并数据"] --> B["计算最终回忆分数"]
    B --> B1["判断是否拒绝回忆"]
    B1 -->|是| B2["最终回忆分数为NA"]
    B1 -->|否| B3["判断是否一个都没回忆起来"]
    B3 -->|是| B4["最终回忆分数为0"]
    B3 -->|否| B5["计算回忆词汇得分"]
    B5 --> C["计算减法测试总分"]
    C --> C1["判断是否使用工具计算"]
...
my_data_wash <- my_data_wash %>%
    mutate(
        final_balance_score = case_when(
            is.na(final_balance_score) & balance_test_affected == 1 ~ 0,
...
```{r}
# 计算最终回忆分数（无误，代填的也会有分，这种怎么处理，暂时不考虑这个条目）
my_data_wash <- my_data_wash %>%
    mutate(
...
my_data_wash <- my_data_wash %>%
    mutate(
        final_left_hand_grip = case_when(
            left_hand_grip_1 == 993 ~ 0,
            left_hand_grip_1 %in% c(999, NA) ~ case_when(
...
my_data_wash <- my_data_wash %>%
    mutate(
        final_right_hand_grip = case_when(
            right_hand_grip_1 == 993 ~ 0,
            right_hand_grip_1 %in% c(999, NA) ~ case_when(
                rowSums(select(., starts_with("grip_test_reason_")) == 4, na.rm = TRUE) > 0 ~ 0,
                rowSums(select(., starts_with("grip_test_reason_")) == 6, na.rm = TRUE) > 0 ~ 0,
                TRUE ~ NA_real_
            ),
...
my_data_wash <- my_data_wash %>%
    mutate(
        final_breath_test = case_when(
            breath_test_1 == 993 ~ 0,
            breath_test_1 %in% c(999, NA) ~ case_when(
