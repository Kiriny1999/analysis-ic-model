...
my_data_calculated <- my_data_calculated %>%
    mutate(
        urban_employee_medical_insurance = case_when(
            my_data_manual_filtered$ea001_w3_2_1_ == 1 ~ 1,  # yes记为1
            my_data_manual_filtered$ea001_w3_2_1_ == 2 ~ 0,  # no记为0
            is.na(my_data_manual_filtered$ea001_w3_2_1_) ~ 0, # NA记为0
            TRUE ~ 0  # 其他情况记为0
        )
    )
...
my_data_calculated <- my_data_calculated %>%
    mutate(
        visithospital_lastmonth = case_when(
            is.na(my_data_manual_filtered$ec004) ~ NA_real_,
            my_data_manual_filtered$ec004 == 1 ~ 1,  # yes记为1
            my_data_manual_filtered$ec004 == 2 ~ 0,  # no记为0
            TRUE ~ NA_real_  # 其他值记为NA
        )
    )
attr(my_data_calculated$visithospital_lastmonth, "label") <- "过去一个月是否住院"
