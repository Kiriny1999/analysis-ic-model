...
my_data_wash <- my_data_wash %>%
    mutate(
        final_walk_time = case_when(
            ...
            TRUE ~ case_when(
                walk_test_aid == 1 ~ case_when(
                    walk_time_second %in% c(993, 999, NA) ~ if_else(2.5 / walk_time_first >= 1, 1, 0),
                    walk_time_first %in% c(993, 999, NA) ~ if_else(2.5 / walk_time_second >= 1, 1, 0),
                    TRUE ~ if_else(2.5 / pmax(walk_time_first, walk_time_second, na.rm = TRUE) >= 1, 1, 0)
                ),
...
my_data_wash <- my_data_wash %>%
    mutate(
        final_feet_together_time = case_when(
            ...
            TRUE ~ if_else(stand_test_feet_together == 1, 1, 0)
        )
    )
...
my_data_wash <- my_data_wash %>%
    mutate(
        final_physical_score = if_else(
            rowSums(is.na(select(., final_walk_time, final_balance_score, final_sit_stand_time))) > 0,
            NA_real_,
            rowSums(select(., final_walk_time, final_balance_score, final_sit_stand_time), na.rm = FALSE)
        )
    )
