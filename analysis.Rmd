    ...
    mutate(
        final_sit_stand_time = case_when(
            ...
            sit_stand_test_completed == 1 ~ case_when(
                ...
                sit_stand_test_arm_use == 2 ~ case_when(
                    sit_stand_test_time <= 12 ~ 1,
                    sit_stand_test_time > 12 ~ 0,
                    TRUE ~ NA_real_
                    ),
                TRUE ~ 0 # 貌似是这个地方NA多，怎么考虑呢
            ),
...
# 计算运动维度总分缺失值数量
missing_physical_score <- sum(is.na(my_data_wash$final_physical_score))
print(paste("运动维度总分缺失值数量:", missing_physical_score))
